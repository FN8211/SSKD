...
if __name__ == '__main__':
    ...
    exp_name = 'sskd_wsld_student_{}_weight{}+{}+{}+{}_T{}+{}+{}_ratio{}+{}_seed{}_{}'.format(
                args.s_arch,
    ...
    for epoch in range(args.epoch):
        ...
        wsld_weight_record = AverageMeter()
        ...
        for x, target in tqdm(train_loader):
            ...
            # ===================== loss2: WSLD加权KD =====================
            # WSLD: 用T=1时学生/教师的CE loss比值算每个样本的权重
            with torch.no_grad():
                log_s1 = F.log_softmax(output[nor_index], dim=1)
                log_t1 = F.log_softmax(knowledge[nor_index], dim=1)
                ce_s = F.nll_loss(log_s1, target, reduction='none')  # [batch]
                ce_t = F.nll_loss(log_t1, target, reduction='none')  # [batch]
                wsld_weight = (ce_s / (ce_t + 1e-7)).clamp(min=0)
                wsld_weight = 1 - torch.exp(-wsld_weight)  # [batch]

            per_sample_kd = F.kl_div(log_nor_output, nor_knowledge, reduction='none').sum(dim=1)  # [batch]
            loss2 = (wsld_weight * per_sample_kd).mean() * args.kd_T * args.kd_T
            # ===============================================================
            ...
            wsld_weight_record.update(wsld_weight.mean().item(), batch)
        ...
        logger.add_scalar('train/wsld_weight', wsld_weight_record.avg, epoch+1)
        logger.add_scalar('train/cls_acc', cls_acc_record.avg, epoch+1)
        logger.add_scalar('train/ss_acc', ssp_acc_record.avg, epoch+1)

        run_time = time.time() - start
        info = 'student_train_Epoch:{:03d}/{:03d}\t run_time:{:.3f}\t ce_loss:{:.3f}\t kd_loss:{:.3f}\t wsld_w:{:.3f}\t tf_loss:{:.3f}\t ss_loss:{:.3f}\t cls_acc:{:.2f}'.format(
            epoch+1, args.epoch, run_time, loss1_record.avg, loss2_record.avg, wsld_weight_record.avg,
            loss3_record.avg, loss4_record.avg, cls_acc_record.avg)
        print(info)
