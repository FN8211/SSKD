...
if __name__ == '__main__':
    ...
    exp_name = 'sskd_student_{}_weight{}+{}+{}+{}+{}_T{}+{}+{}_ratio{}+{}_seed{}_{}'.format(
                args.s_arch,
                args.ce_weight, args.kd_weight, args.tf_weight, args.ss_weight, args.tri_weight,
    ...
    for epoch in range(args.epoch):
        ...
        for x, target in tqdm(train_loader):
            ...
            # ===================== loss5: triplet loss =====================
            # s_simi, t_simi shape = [3*batch, batch]
            # aug_feat[i] 对应 nor_feat[i//3]，即正样本 index = i//3
            pos_idx = torch.arange(3*batch) // 3  # [0,0,0,1,1,1,...]

            # 正样本相似度
            s_pos_sim = s_simi[torch.arange(3*batch), pos_idx]  # [3*batch]
            t_pos_sim = t_simi[torch.arange(3*batch), pos_idx]  # [3*batch]

            # mask 掉正样本位置，取 batch 内最难负样本
            mask = torch.zeros(3*batch, batch, dtype=torch.bool).cuda()
            mask[torch.arange(3*batch), pos_idx] = True
            s_neg_sim = s_simi.masked_fill(mask, -1e9).max(dim=1)[0]  # [3*batch]
            t_neg_sim = t_simi.masked_fill(mask, -1e9).max(dim=1)[0]  # [3*batch]

            # loss5 = max(0, (s_neg - s_pos) - (t_neg - t_pos))
            # 要求学生的正负间距至少达到教师水平
            loss5 = F.relu(
                (s_neg_sim - s_pos_sim) - (t_neg_sim - t_pos_sim).detach()
            ).mean()
            # ===============================================================
            ...
            loss5_record.update(loss5.item(), 3*batch)
        ...
        logger.add_scalar('train/tri_loss', loss5_record.avg, epoch+1)  # 新增
        logger.add_scalar('train/cls_acc', cls_acc_record.avg, epoch+1)
        logger.add_scalar('train/ss_acc', ssp_acc_record.avg, epoch+1)

        run_time = time.time() - start
        info = 'student_train_Epoch:{:03d}/{:03d}\t run_time:{:.3f}\t ce_loss:{:.3f}\t kd_loss:{:.3f}\t tri_loss:{:.3f}\t cls_acc:{:.2f}'.format(
            epoch+1, args.epoch, run_time, loss1_record.avg, loss2_record.avg, loss5_record.avg, cls_acc_record.avg)
        print(info)
