"""
student_examplar.py — Exemplar SSKD for Tiny ImageNet (vgg13 → vgg8).
SS task: predict dataset index (instance discrimination).
num_data = 100000 (Tiny ImageNet train size).
"""
import os
...
from tin_exemplar import TinyImageNetExemplar
from tiny_imagenet import TIN_MEAN, TIN_STD

torch.backends.cudnn.benchmark = True

parser = argparse.ArgumentParser(description='Tiny ImageNet SSKD student (Exemplar).')
...
class wrapper_exemplar(nn.Module):
    def __init__(self, module, num_data=100000):
        super().__init__()
        self.backbone = module
        self.proj_head = nn.Linear(module.classifier.in_features, num_data)
...
if __name__ == '__main__':
    ...
    exp_name = 'tin_sskd_ex_student_{}_weight{}+{}+{}+{}_T{}+{}+{}_ratio{}_{}'.format(
        args.s_arch,
    ...
    transform_aug = transforms.Compose([
        transforms.RandomResizedCrop(64, scale=(0.08, 1.0), ratio=(3/4, 4/3)),
    ...
    trainset = TinyImageNetExemplar(args.data_root, train=True,
                                    transform=transform_train,
                                    transform_aug=transform_aug)
    valset   = TinyImageNetExemplar(args.data_root, train=False,
    ...
    for epoch in range(args.t_epoch):
        ...
        for x, _, data_index in tqdm(train_loader):
            ...
            acc_record.update(accuracy(ex_logit, ex_labels, topk=(1,))[0].item(), x_all.size(0))
    ...
    s_model = wrapper_exemplar(
        module=model_dict[args.s_arch](num_classes=200),
        num_data=args.num_data).cuda()
    ...
    for epoch in range(args.epoch):
        ...
        for x, target, data_index in tqdm(train_loader):
            ...
            loss4 = F.kl_div(log_s_ex, t_ex_soft, reduction='batchmean') * args.ss_T ** 2
            ...
            ssp_acc_record.update(accuracy(s_ex_logit, ex_labels, topk=(1,))[0].item(), 4 * batch)
