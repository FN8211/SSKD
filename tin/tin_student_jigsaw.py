"""
student_jigsaw.py — Jigsaw SSKD for Tiny ImageNet (vgg13 → vgg8).
"""
import os
...
from tin_jigsaw import TinyImageNetJigsaw
...
class wrapper_jigsaw(nn.Module):
    def __init__(self, module):
        ...
        self.proj_head = nn.Linear(module.classifier.in_features, 24)
...
if __name__ == '__main__':
    ...
    exp_name = 'tin_sskd_jig_student_{}_weight{}+{}+{}+{}_T{}+{}+{}_ratio{}_{}'.format(
        args.s_arch,
    ...
    trainset = TinyImageNetJigsaw(args.data_root, train=True,  transform=transform_train)
    valset   = TinyImageNetJigsaw(args.data_root, train=False, transform=transform_test)
    ...
    for epoch in range(args.t_epoch):
        ...
        for x, _, perm_indices in tqdm(train_loader):
            ...
            jig_labels = perm_indices.view(-1).cuda()
            _, jig_logit, _ = t_model(x_all, bb_grad=False)
            loss = F.cross_entropy(jig_logit, jig_labels)
            loss.backward()
            t_optimizer.step()
            loss_record.update(loss.item(), x_all.size(0))
            acc_record.update(accuracy(jig_logit, jig_labels, topk=(1,))[0].item(), x_all.size(0))
    ...
    s_model = wrapper_jigsaw(module=model_dict[args.s_arch](num_classes=200)).cuda()
    ...
    for epoch in range(args.epoch):
        ...
        for x, target, perm_indices in tqdm(train_loader):
            ...
            loss4 = F.kl_div(log_s_jig, t_jig_soft, reduction='batchmean') * args.ss_T ** 2
            ...
            ssp_acc_record.update(accuracy(s_jig_logit, jig_labels, topk=(1,))[0].item(), 4 * batch)
