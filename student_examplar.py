...
from cifar_exemplar import CIFAR100Exemplar
...
parser = argparse.ArgumentParser(description='train SSKD student network (Exemplar).')
...
# total number of training samples, used for Exemplar classifier size
parser.add_argument('--num-data', type=int, default=50000)
...
# ── wrapper_exemplar: SS module is a num_data-way classifier ──
class wrapper_exemplar(nn.Module):
    """
    Backbone wrapper that adds a num_data-way Exemplar classifier as SS module.
    feat_dim is auto-detected from backbone.classifier.in_features.
    forward returns: (main_logit, exemplar_logit, feat)
    """
    ...
    def forward(self, x, bb_grad=True):
        if bb_grad:
            ...
            with torch.no_grad():
                feats, logit = self.backbone(x, is_feat=True)
        feat = feats[-1]
        ex_logit = self.proj_head(feat)
        return logit, ex_logit, feat
...
if __name__ == '__main__':
    ...
    exp_name = 'sskd_exemplar_student_{}_weight{}+{}+{}+{}_T{}+{}+{}_ratio{}_seed{}_{}'.format(
                args.s_arch,
    ...
    # heavy augmentation for exemplar views (index 1-3)
    # following Sec 6.2: color jitter + random crop + resize
    ...
    # CIFAR100Exemplar returns (imgs, target, index)
    # imgs: (N, 4, C, H, W), index: (N,) dataset indices
    trainset = CIFAR100Exemplar('./data', train=True,
                                transform=transform_train,
                                transform_aug=transform_aug)
    valset   = CIFAR100Exemplar('./data', train=False,
                                transform=transform_test,
                                transform_aug=transform_test)
    ...
    # ── load teacher backbone and wrap with exemplar head ──
    ...
    t_model = wrapper_exemplar(module=t_model, num_data=args.num_data).cuda()
    ...
    # ── Stage 1: train teacher exemplar head (backbone frozen) ──
    for epoch in range(args.t_epoch):
        ...
        start = time.time()
        for x, _, data_index in tqdm(train_loader):
            t_optimizer.zero_grad()

            c, h, w = x.size()[-3:]
            x_all = x.view(-1, c, h, w).cuda()             # (4N, C, H, W)

            # exemplar label for each view = dataset index of its original image
            # repeat index 4 times to match 4 views
            ex_labels = data_index.unsqueeze(1).expand(-1, 4).reshape(-1).cuda()  # (4N,)

            _, ex_logit, _ = t_model(x_all, bb_grad=False)
            loss = F.cross_entropy(ex_logit, ex_labels)

            loss.backward()
            t_optimizer.step()

            batch_acc = accuracy(ex_logit, ex_labels, topk=(1,))[0]
        ...
        if (epoch + 1) % args.val_interval == 0:
            ...
            loss_record = AverageMeter()
            start = time.time()
            for x, _, data_index in tqdm(val_loader):
                c, h, w = x.size()[-3:]
                x_all = x.view(-1, c, h, w).cuda()
                ex_labels = data_index.unsqueeze(1).expand(-1, 4).reshape(-1).cuda()
                with torch.no_grad():
                    _, ex_logit, _ = t_model(x_all)
                loss = F.cross_entropy(ex_logit, ex_labels)
                batch_acc = accuracy(ex_logit, ex_labels, topk=(1,))[0]
                acc_record.update(batch_acc.item(), x_all.size(0))
                loss_record.update(loss.item(), x_all.size(0))
    ...
    s_model = wrapper_exemplar(module=s_model, num_data=args.num_data).cuda()
    ...
    for epoch in range(args.epoch):
        ...
        start = time.time()
        for x, target, data_index in tqdm(train_loader):
            optimizer.zero_grad()
            ...
            s_aug_out, s_ex_logit,  _ = s_model(x_all, bb_grad=True)
            # s_output   : (N, 100)      main cls on normal images
            # s_aug_out  : (4N, 100)     main cls on all views (for LT)
            # s_ex_logit : (4N, 50000)   exemplar logits on all views

            with torch.no_grad():
                t_output,  _,           _ = t_model(x_nor)
                t_aug_out, t_ex_logit,  _ = t_model(x_all)
            ...
            # ── loss4: exemplar SS distillation (Lss) ──
            # match student and teacher exemplar logits via KL divergence
            # note: 50000-dim softmax is computed here; expensive but faithful to paper
            t_ex_soft = F.softmax(t_ex_logit / args.ss_T, dim=1).detach()
            log_s_ex  = F.log_softmax(s_ex_logit / args.ss_T, dim=1)
            loss4 = F.kl_div(log_s_ex, t_ex_soft,
            ...
            ssp_batch_acc = accuracy(s_ex_logit, ex_labels, topk=(1,))[0]
